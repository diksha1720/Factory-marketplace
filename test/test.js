...
describe("Game Contract Test Cases", () => {
    ...
    it("should create an event", async() =>{
        ...
        await expect(contractFactory.connect(organizer).createEvent(eventId , "event1", "evt1" , "1000000", 100, "10000000", '0x'+ rootHash)).to.be.revertedWithCustomError(contractFactory ,"GlobalVarNotUpdated")
        ...
        var txn = await contractFactory.connect(organizer).createEvent(eventId , "event1", "evt1" , "1000000", 100, "10000000", '0x'+ rootHash)
    ...
    it("should mint tokens from primary market place", async()=>{
        ...
        await expect(primaryMarketPlace.connect(account6).mintTokens(eventId,"1000000",proof )).to.be.revertedWithCustomError(primaryMarketPlace ,"UserNotWhiteListed")

        var proof = getMerkleProof(merkleTreeMain , account2.address)
   
        await expect(primaryMarketPlace.connect(account2).mintTokens(eventId,"10000000000000",proof )).to.be.revertedWithCustomError(primaryMarketPlace ,"AllowanceExceeded")
    ...
    it("should mint nft from the secondary marketplace", async()=>{
        ...
        await  expect(primaryMarketPlace.connect(account6).mintNFT(eventId, 1 , proof)).to.be.revertedWithCustomError(primaryMarketPlace ,"UserNotWhiteListed")

        await  expect(primaryMarketPlace.connect(account2).mintNFT(eventId, 1000 , proof)).to.be.revertedWithCustomError(primaryMarketPlace , "AllowanceExceeded")

        await  expect(primaryMarketPlace.connect(account2).mintNFT(eventId, 100 , proof)).to.be.revertedWithCustomError(primaryMarketPlace  , "InsufficientFunds")
    ...
    it("should resell the minted NFT on secondary marketplace", async() => {
        ...
        await expect(secondaryMarketPlace.connect(account3).listNFT(eventId, tokenId , "10000")).to.be.revertedWithCustomError(secondaryMarketPlace , "InvalidOwner")
    ...
    it("another user should be able to buy the listed NFTs ", async() =>{
        await expect(secondaryMarketPlace.connect(account6).buyNFT(nftContract , tokenId + 1 , {value :"10000" })).to.be.revertedWithCustomError(secondaryMarketPlace ,"NftNotListed")
    ...
    it("end user can list the bought NFT to the secondary marketplace again", async function(){
        ...
        await expect(secondaryMarketPlace.connect(account6).listNFT(eventId, tokenId , "1000000")).to.be.revertedWithCustomError(secondaryMarketPlace , "PriceExceedsPrevPrice")
